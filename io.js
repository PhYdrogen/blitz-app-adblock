...
function modifyFileAtContext  (data, filePath, context) {
    ...
    if (file[line] !== data) {
      for (let j = line; j < file.length; j++) {
          console.log(file[j]);

          if (file[j].trim() == ");") { //file.indexOf(");")
            console.log("TROUVER");
            del = j - line + 1;
            break
          }
...
module.exports = {
    ...
    modifyFileAtContext: modifyFileAtContext,
